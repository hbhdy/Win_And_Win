...
public class HulkBossCtrl : MonoBehaviour
{
    ...
    // 공격 사정거리
    public float attackDistPowerHook = 15.0f;
    // 네비메쉬를 사용하기 위함
    ...
    public IEnumerator CheckZombieState()
    {
        while (!isDie)
        {
            ...
            //yield return null;
            realdist = Vector3.Distance(playerTransform.position, hulkTransform.position);
            if (realdist <= attackDistPowerHook)
            {
                hulkState = HulkState.attack;
            }
            //else if (realdist <= traceDist) // 추적거리 범위 이내로 들어왔는지 확인
            //{
            //    hulkState = HulkState.walk;
            //}
            else if (realdist > traceDist)
            {
    ...
    public IEnumerator ZombieAction()
    {
        while (!isDie)
        {
            ...
            {
                case HulkState.idle:
                    hulkTransform.LookAt(playerTransform);
